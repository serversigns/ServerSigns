...
public class Updater {
        ...
        if (this.type != UpdateType.NO_VERSION_CHECK) {
            ...
            if (title.split(DELIMETER).length >= 2) {
                ...
                if (this.hasTag(localVersion) || !this.shouldUpdate(localVersion, remoteVersion)) {
                    ...
                    // Check if our version is higher than the one we're trying to download
                    if (!requiresNewVersion(localVersion, remoteVersion)) {
                        // We're already ahead of the curve
                        this.plugin.getLogger().info("Skipping plugin update as you are ahead of the curve!");
                        this.result = Updater.UpdateResult.NO_UPDATE;
                        return false;
                    }
                }
    ...
    /**
     * Verify if version is higher than last version to prevent override of version
     *
     * @param currentVersion Current version
     * @param remoteVersion Bukkit last versoon
     * @return true if version is not higher than bukkit version
     */
    private boolean requiresNewVersion(String currentVersion, String remoteVersion) {
        String[] split = currentVersion.split("\\.");
        int[] currentVerInts = new int[split.length];

        for (int k = 0; k < split.length; k++) {
            if (NumberUtils.isInt(split[k])) {
                currentVerInts[k] = NumberUtils.parseInt(split[k]);
            } else {
                return false; // We're on a special version, don't update
            }
        }

        split = remoteVersion.split("\\.");
        int[] remoteVerInts = new int[split.length];

        for (int k = 0; k < split.length; k++) {
            remoteVerInts[k] = NumberUtils.parseInt(split[k], -1);
        }

        int max = currentVerInts.length > remoteVerInts.length ? remoteVerInts.length : currentVerInts.length;
        for (int k = 0; k < max; k++) {
            if (remoteVerInts[k] > currentVerInts[k]) {
                // We're behind, update
                return true;
            }
        }
